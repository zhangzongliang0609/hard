...
var Kalendae = function (targetElement, options) {
	...
	util.addEvent($container, 'mousedown', function (event, target) {
		...
		//DAY CLICK
			 ...
			 * by linquan
			 * date 2019-09-16
			 * 增加选择完日期后，关闭日历(范围类型 需要点两下)
			* */
			if(opts.mode ==='range' && opts.clickHide){
				if(self.getSelected().length > 20){
					self.hide();
					self.input.blur();
				}
			}else
			 ...
			 * 增加选择完日期后，关闭日历
			 */
			// console.log('clickHide' in opts);
			if(opts.clickHide) {
				// console.log('day-click: hide');
				self.hide();
				self.input.blur(); // 让当前input主动失去焦点，下此次点击会在此获取焦点从而显示日期选择器
			}
			return false;
...
Kalendae.prototype = {
	defaults : {
		...
		dateClassMap          :{},
		clickHide             :false 			/* boolean to use select dates then hide kalendae */
	},
	classes : {
		container       :'kalendae',
		calendar        :'k-calendar',
...
// ELEMENT FUNCTIONS
	...
	isFixed: function (elem) {
		do {
			if (util.getStyle(elem, 'position') === 'fixed') return elem;
...
Kalendae.Input.prototype = util.merge(Kalendae.prototype, {
	...
	show : function () {
		...
		//
		// var	scrollTop = $scrollContainer == style.position ? $scrollContainer.scrollTop : 0,
		// 	scrollLeft = $scrollContainer == style.position ? $scrollContainer.scrollLeft : 0,
		// 	opts = this.settings;
		style.display = '';
