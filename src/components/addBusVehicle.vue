            ...
            <form id="J-layer-form" class="form form-horizontal clearfix">
                <div class="f-l col" :class="{'col-4': isShowRightPart, 'col-12': !isShowRightPart}">
                    ...
                    <div>
                        ...
                        <div class="row clearfix">
                            <label class="form-label f-l col-3"><span class="c-red">*</span>绑定线路</label>
                            <div class="formControls f-l col-9 clearfix line_box">
                                <div class="f-l col-5">
                                    <div class="line_container">
                                        <div @click="selectAvailableLine(item)"
                                             :class="{'selected': isSelectedAvailableLines(item)}" class="pointer line"
                                             v-for="item in availableLines" :title="item.shuttle_name">
                                            {{item.shuttle_name}}
                                        </div>
                                    </div>
                                </div>
                                <div class="f-l col-2 btns_flex">
                                    <a href="javascript:;" class="btn btn-primary radius" @click="addBindLines">添加</a>
                                    <a href="javascript:;" class="btn btn-primary radius mt-10"
                                       @click="deleteBindLines">删除</a>
                                </div>
                                <div class="f-l col-5">
                                    <div class="line_container">
                                        <div @click="selectBindLine(item)"
                                             :class="{'selected': isSelectedBindLines(item)}" class="pointer line"
                                             v-for="item in bindLines" :title="item.shuttle_name">{{item.shuttle_name}}
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="row clearfix">
                            <label class="form-label f-l col-3">车辆分组</label>
                            <div class="formControls f-l col-6">
    ...
    export default{
        ...
        data: function () {
            return {
                ...
                tagsArr: [],
                lines: [], // 所有的线路
                availableLines: [], // 可加入绑定的线路
                selectedAvailableLines: [], // 选择的可加入绑定的线路
                bindLines: [], // 已经加入绑定的线路
                selectedBindLines: [], // 选择的已经加入绑定的线路
                types: [], // 车辆类型
                type_name: '', // 选择的车辆类型名称
...
//类型判断
                if (this.type == 'add') {
                    ...
                    ajaxUrl = `${base.gAjaxUrl.shuttlebusvehicles}${this.data.id}/`;
                    ajaxType = 'PUT';
                    ...
                    this.bindLines = getData.shuttlebus_set || [];
                }
...
//					},
                    ...
                    submitHandler: function () {
                        ...
                        let shuttlebus = that.getBindLineIds();
                        if(type == '') {
                            layer.msg("请选择正确的车辆类型", {icon: 2, time: 1500});
                            return;
                        }
                        if(!shuttlebus.length) {
                            layer.msg("请至少绑定一条线路", {icon: 2, time: 1500});
                            return;
                        }
                        let data = {
                            car_no: that.car_no,
...
//                            maintenance_deadline: that.maintenance_deadline || '',
            ...
            addBindLines: function () {
                this.bindLines = this.bindLines.concat(this.selectedAvailableLines);
                this.selectedAvailableLines = [];
            },
            deleteBindLines: function () {
                for (let i = 0, len = this.selectedBindLines.length; i < len; i++) {
                    let item = this.selectedBindLines[i];
                    let index = this.bindLines.indexOf(item);
                    if (index > -1) {
                        this.bindLines.splice(index, 1)
                    }
                }
                this.selectedBindLines = [];
            },
            setAvailableLines: function () {
                let arr = [];
                for (let i = 0, len = this.lines.length; i < len; i++) {
                    let item = this.lines[i];
                    if (!this.isBindLine(item)) {
                        arr.push(item)
                    }
                }
                this.availableLines = arr;
            },
            // 判断是否在绑定线路中
            isBindLine: function (line) {
                for (let i = 0, len = this.bindLines.length; i < len; i++) {
                    if (line.id === this.bindLines[i].id) {
                        return true
                    }
                }
                return false
            ...
            // 选择可选线路
            selectAvailableLine: function (line) {
                let index = this.selectedAvailableLines.indexOf(line);
                if (index > -1) {
                    this.selectedAvailableLines.splice(index, 1)
                } else {
                    this.selectedAvailableLines.push(line);
                }
            },
            // 选择已选的绑定线路
            selectBindLine: function (line) {
                let index = this.selectedBindLines.indexOf(line);
                if (index > -1) {
                    this.selectedBindLines.splice(index, 1)
                } else {
                    this.selectedBindLines.push(line);
                }
            },
            isSelectedAvailableLines: function (line) {
                return this.selectedAvailableLines.indexOf(line) > -1;
            },
            isSelectedBindLines: function (line) {
                return this.selectedBindLines.indexOf(line) > -1;
            },
            getBindLineIds: function (line) {
                let arr = [];
                for(let i = 0, len = this.bindLines.length; i < len;i++) {
                    arr.push(this.bindLines[i].id)
                }
                return arr
            },
            // 车辆类型搜索 -- 开始
        ...
        watch: {
            'lines': function (n, o) {
                this.setAvailableLines();
            },
            'bindLines': function (n, o) {
                this.setAvailableLines();
            },
