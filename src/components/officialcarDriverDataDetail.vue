        ...
        <div class="layui-layer-content layui-layer-content2">
            <div id="mainDriver" class="main-driver">
                <div class="driver-detail">
                    <ul>
                        <li class="f-l text-c">
                            <img src="../assets/images/driver_header.jpg" alt="" class="f-l">
                            <div class="f-l">司机：&nbsp;{{driverDate.real_name}}</div>
                        </li>
                        <li class="f-l text-c">
                        
                            <div>车辆品牌：&nbsp;{{vehicle_vo.car_brand?vehicle_vo.car_brand:"--"}}</div>
                        </li>
                        <li class="f-l text-c">
                        
                            <div>工号：&nbsp;{{driverDate.job_number}}</div>
                        </li>
                        <li class="f-l text-c">
                        
                            <div>电话：&nbsp;{{driverDate.phone}}</div>
                        </li>
                    </ul>
                </div>
...
//            $("#selectDayId option").eq(driverSelectedIndex).attr("selected","selected").siblings("option").removeAttr("selected");
//            let value = $("#selectDayId option").eq(driverSelectedIndex).val();
//            this.days = value;
...
//                this.end_date = this.formatDate(0);
            ...
            getOrders: function () {
                let that = this;
                $.ajax({
                    url: base.gAjaxUrl.officialcardrivers + that.data + '/orders/',
                    type: 'GET',
                    data:{
            ...
            getBaseData: function () {
                let that = this;
                $.ajax({
                    url: base.gAjaxUrl.officialcardrivers + that.data + '/driver_profile/',
                    type: 'GET',
                    data:{
                        start_date: that.start,
                        end_date: that.end,
                ...
                .success((data)=>{
                    that.loading = false;
                    that.driverDate = data.driver.staff_vo;
                    that.vehicle_vo = data.driver.vehicle_vo?data.driver.vehicle_vo:{};
