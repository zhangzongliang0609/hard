...
<div class="layui-layer layui-anim layui-layer-page layui-modify">
    <div class="layui-layer-title">材料类型批量导出</div>
    <div class="layui-layer-content">
        <a href="javascript:;" class="btn btn-primary ml-10 radius" @click="selectAll">全选</a>
        <div class="material_container clearfix">
            <div track-by="$index" @click="add(item.id)" :class="{'selected': isSelected(item.id)}" class="f-l material ml-10 mt-10" v-for="item in items">{{item.name}}</div>
        </div>
    </div>
