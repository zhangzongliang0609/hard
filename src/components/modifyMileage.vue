...
<script>
    ...
    export default {
        ready(){
            ...
            this.mileage = (this.data.order_mileage > 0 ? this.data.order_mileage / 1000 : this.data.mileage / 1000).toFixed(2);
            //验证
            $('#J-from').validate({
                rules: {
                    mileage: {
        ...
        methods:{
            ...
            save:function(){
                $.showLoading();
                $.ajax({
                    url:`${base.gAjaxUrl.privateCarOrders}${this.data.id}/`,
                    type:"PATCH",
                    data:{
                        order_mileage:this.mileage * 1000
                    }
                }).always((data)=>{
                    $.hideLoading();
