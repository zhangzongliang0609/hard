<template>
    ...
    <enter v-if="showEnter" v-on:close="closeEnter" :costlist="costTypeList" @dialog-save="saveCost"></enter>
        ...
        <div class="page-container bgc-f5">
            <div class="panel panel-default radius">
                <div class="panel-body clearfix">
                    ...
                    <a class="btn btn-secondary radius f-r mr-20" title="费用类型管理" href="javascript:;" v-on:click="exportData()">费用导出</a>
                    <a class="btn btn-secondary radius f-r mr-20" title="费用录入" href="javascript:;" v-on:click="inputPrice()">费用录入</a>
                ...
                <div class="materials bk-gray radius bgc-fff">
                    <table class="waterElectricityForm table table-border table-bg table-striped bor-l bor-r">
                        ...
                        <tr class="text-c" v-for="data in items">
                            ...
                            <td>{{data.jz_date}}</td>
                            <td>
                                <img class="item-img" :src="data.img?data.img:'../../assets/images/'" @click="clickImg(data.img)" >
                            </td>
                            <td>{{data.operator}}</td>
    ...
    export default{
        ...
        data:function () {
            return{
                itemsLoading:false,
                title:'费用类型管理',
                tagUrl:`${base.gAjaxUrl.officialcarCostTypes_V2}`, // 类别管理url
                showSetTag:false,
                noData:false,
                orderStatusSelected:'',
        ...
        methods:{
            ...
            queryAjax:function(callback=function(){}){
                ...
                //修改参数
                $.ajax({
                    url: `${base.gAjaxUrl.officialcarCost_V2}`,
                    type: 'GET',
                    data:obj
                }).always((data)=>{
            ...
            confirmCost:function (item) {
                ...
                let that = this;
                layer.confirm(`车牌号码:${item.car_no},&nbsp;驾驶人:${item.driver_name},&nbsp;费用类型:${item.feetype_name},&nbsp;费用金额:${item.fee / 100},此费用是否已确认?`,function(){
                    $.ajax({
                        url: `${base.gAjaxUrl.officialcarCost_V2}${item.id}/confirm/`,
                        type: 'GET',
                    }).always((data)=>{
                        base.ajaxCallback(data,()=>{
                            that.getCostList();
            ...
            deleteCost:function (item) {
                let that = this;
                layer.confirm(`车牌号码:${item.car_no},&nbsp;驾驶人:${item.driver_name},&nbsp;费用类型:${item.feetype_name},&nbsp;费用金额:${item.fee / 100 },是否确定作废此费用?`,function(){
                    $.ajax({
                        url: `${base.gAjaxUrl.officialcarCost_V2}${item.id}/discard/`,
                        type: 'GET',
                    }).always((data)=>{
                        base.ajaxCallback(data,()=>{
                            that.getCostList();
            ...
            clickImg:function(data){
                this.globalImageSrc = data;
            },
            ...
            exportData: function () {
                ...
                obj[this.searchType] = this.searchContent;
                $.ajax({
                    url: `${base.gAjaxUrl.officialcarCost_V2}export/`,
                    type:'GET',
                    data:obj,
                }).always((data)=>{
                    $.hideLoading();
