<template>
    <div class="page-container">
        ...
        <table class="table table-border table-bordered table-hover table-bg table-sort">
            ...
            <tr class="text-c" v-for="item in items">
                ...
                <td>
                   {{item.maintenance_deadline}}
                </td>
                <td :class="{'text-red': item.maintenance_rest_days <= 30}">{{item.maintenance_rest_days <= 0 ? "已过期" : item.maintenance_rest_days}} </td>
                <td><img class="warning" v-show="item.maintenance_rest_days<=30 && item.maintenance_rest_days || item.maintenance_rest_days == 0 " src="../../assets/images/warning.png" alt=""></td>
                <td class="f-14">
                    <a href="javascript:;" class="btn btn-primary radius mr-5" @click="showRecord(item)">确认已保养</a>
                </td>
            </tr>
            </tbody>
        </table>
        <div id="M-page" class="m-page" v-if="!noData"></div>
    ...
    <maintenance v-on:window-close="close" v-if="isShowWindow" :data="data" :type="portType"  v-on:child-data="updata"></maintenance>
</template>
    ...
    export default{
        ready(){
            ...
            this.queryAjax((tpageToal='4',count='0')=>{
                setTimeout(function () {
                    that.initPage('#M-page',tpageToal,staffsOps.param.page,count)
                },100);
            });
        },
        data:function () {
