            ...
            <div class="operation bk-gray radius bgc-fff">
                <div class="car_status clearfix">
                    <div class="car_all f-l" @click="setStatus(0)">
                        <div class="number tc lh-42 f-26" :class="{'c-09f': currentStatus == 0}">{{summary ? summary.count : 0}}
                        </div>
                    ...
                    <div class="car_online f-l" @click="setStatus(1)">
                        <div class="number tc lh-42 f-26" :class="{'c-0c9': currentStatus == 1}">{{summary ? summary.online : 0}}
                        </div>
                        <div class="txt f-16 lh-30">
                    ...
                    <div class="car_offline f-l" @click="setStatus(2)">
                        <div class="number tc lh-42 f-26" :class="{'c-ae': currentStatus == 2}">{{summary ? summary.offline : 0}}
                        </div>
                        <div class="txt f-16 lh-30">
...
<script type="text/ecmascript-6">
    ...
    import exp from "../components/GJexportLocation.vue";
...
//							console.log("show -- map load");
                ...
                //默认
                let defaulted={}
                defaulted.car_id=localStorage.bus_id
                defaulted.date=localStorage.driver_date

                if(defaulted.car_id != null){
                    this.type=2;
                    this.getTrack(defaulted)
                }
            },
            ...
            setInfoWin: function (item) {
                ...
                this.$currentInfoWinDom.find(".driver").text(item.driver_name || '--');
                this.$currentInfoWinDom.find(".phone").text(item.driver_phone || '--');
            ...
            moveToPoint: function (item) {
                ...
                $.ajax({
                    ...
                    base.ajaxCallback(real, () => {
                        // 实时/最新的位置信息
//                        this.setInfoWin($.extend({}, item, real.latest_location));
                        // 实时/最新的位置信息 -- 并同时更新左侧列表显示的速度
                        this.setInfoWin($.extend(item, real.latest_location));
                    });
                });
            },
                ...
                $.ajax({
                    ...
                    base.ajaxCallback(data, () => {
                        this.baiduTrailInstance.setOptions({
                            ...
                            driver: this.currentCarItem.staff_vo,
                            vehicle: this.currentCarItem
                        });
                        localStorage.removeItem("bus_id")
                        localStorage.removeItem("driver_date")
                        this.type=1
                    });
...
<style lang="less" rel="stylesheet/less" scoped>
    ...
    .carInfoHeader0 {
        background-image: url("/static/images/icon-bus-online.png");
        background-size: 18px 20px;
    }
    .carInfoHeader1 {
        background-image: url("/static/images/icon-bus-offline.png");
        background-size: 18px 20px;
    }
</style>
