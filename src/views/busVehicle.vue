        ...
        <div class="page-container">
            ...
            <div class="panel panel-default radius clearfix mt-10 mb-10">
                <div class="panel-body clearfix">
                    ...
                    <a class="btn btn-primary radius mr-15 f-r" href="javascript:;" v-on:click="turnToBusType()">车辆类型管理</a>
                </div>
            </div>
            <table class="table table-border table-bordered table-hover table-bg table-sort">
                ...
                <tr class="text-c" v-for="item in items">
                    ...
                    <th>
                        <span v-if="item.shuttlebus_set.length == 0">未绑定线路</span>
                        <span v-else>{{item.shuttlebus_set | getLines}}</span>
                    </th>
                    <td>
    ...
    export default{
        ...
        methods: {
            ...
            toggleLock: function (busVehicle) {
                $.showLoading();
                $.ajax({
                    url: base.gAjaxUrl.shuttlebusvehicles + busVehicle.id + '/lock/',
                    type: 'PATCH',
            ...
            getList: function (id = 0) {
                if (id) {
                    ...
                    staffsOps.param.car_no = '';
                    staffsOps.param.type_name = '';
                    staffsOps.param[this.searchType] = $('#searchInput').val();
                }
            ...
            exportData: function () {
                $.showLoading();
                $.ajax({
                    url: `${base.gAjaxUrl.shuttlebusvehicles}export/`,
                    type: 'POST',
                    contentType: "application/json",
                    data: JSON.stringify({
