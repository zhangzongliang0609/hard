        ...
        <div class="page-container bgc-f5">
            <div class="panel panel-default radius">
                <div class="panel-body">
                    <span class="f-20 fw-b">车辆故障项目管理</span>
                </div>
            </div>
                    ...
                    <div class="firstItems f-l">
                        <div class="tabBar clearfix">
                            ...
                            <div class="clearfix text-c" v-for="item in faultitems" role="tab" v-on:click="clickTab($index)" v-bind:class="[item.index==itemIndex?'curr':'']">
                                <span class="items">
                                    <span title="{{item.fault_name}}" class="itemName text-overflow">{{item.fault_name}}</span>
                                    <a class="modify" href="javascript:;" v-on:click="modifyFault(item,'firstItem')" v-show="isShowModify"><i class="Hui-iconfont">&#xe6df;</i></a>
                                    <a class="delete" href="javascript:;" v-on:click="deleteFault(item,'firstItem')" v-show="isShowDelete"><i class="Hui-iconfont">&#xe6a6;</i></a>
                                </span>
...
<script>
    ...
    import add from "../components/addVehicleFaultItem.vue";
...
//                console.log('getList')
            ...
            queryAjax:function(callback=function(){}){
                ...
                $.ajax({
                    url: base.gAjaxUrl.vehicleFaultitems,
                    type: 'GET',
            ...
            deleteFault:function(item,type){
                ...
                if(type == 'firstItem'){
                    title = '删除大分类'
                    temple = `现在要删除故障项目大分类：${item.fault_name}<br/>将会删除大分类${item.fault_name}下所包含的所有故障项目小分类  共${item.items?item.items.length:0}项`;
                    ajaxData = {
                    }
                }else if(type == 'secondItem'){
                    title = '删除小分类'
                ...
                layer.confirm(
                    ...
                    (index)=>{
                        $.ajax({
                            url: `${base.gAjaxUrl.vehicleFaultitems}${item.id}/`,
                            type: 'DELETE',
                            data: ajaxData
                        })
                        .always((data)=>{
