        ...
        <div id="mContainer" class="page-container bgc-f5 bgc-f5">
            <div class="panel panel-default radius">
                <div class="panel-body">
                    <span class="f-15 fw-b">站点设置</span>
                </div>
            </div>
            <div class="station_list_box bk-gray radius bgc-fff">
                ...
                <div class="text-c pl-10 pr-10">
                    <div @click="clickInput" class="mt-10 mb-10"><input :disabled="!isCompile"   type="text" placeholder="请输入新的站点名称" class="input-text text-c radius add-station newStation"><img v-show="isCompile"  class="img-icon add" src="../assets/images/icon-plus.png" alt="" @click="add"></div>
                    <p class="text-c f-12">拖拽站点可改变站点顺序↓</p>
                    <div  class="item_content">
                    <ul class="stationLists">
                        <li v-for="(index, station) in busStations">
                            <div v-if="true" class="item" v-on:click="moveToPoint($event, index, station)" @mouseup="itemDrag">

                                <div class="img-icon delete c-white" @click="deleteStation($index,station.station_name)" v-show="inputShow && $index == inputIndex"><i class="icon Hui-iconfont">&#xe6a6;</i></div>
                                <span class="item-name" v-hidden="$index == inputIndex && inputShow"  @click="">{{station.station_name}}</span>
                                <input type="text" v-model="station.station_name" v-show="inputShow && $index == inputIndex">

                                <div class="img-icon sure c-white" @click="saveStation(station.station_name)" v-show="inputShow && $index == inputIndex"><i class="icon Hui-iconfont">&#xe6a7;</i></div>
                                <div class="img-icon change c-333" @click="showSaveBtn($index)" v-show="isCompile && !inputShow"><i class="icon Hui-iconfont">&#xe6df;</i></div>
                    ...
                    <div >
                        <a href="javascript:;" class="btn btn-primary radius mt-20" v-on:click="compile()" v-show="!isCompile">编辑</a>
                        <a href="javascript:;" class="btn btn-primary radius mt-20" v-on:click="clickSave()" v-show="isCompile">保存</a>
                    </div>
                </div>
            ...
            <div id="map_pinba">
                <div class="clearfix" style="position: relative; z-index: 1000;">
                    <span class="select-box bgc-f4 relative f-l" style="padding:0">
                            ...
                            <!--<div class="col-12 f-l">-->
                                ...
                                <!--<div class="formControls f-l col-7">-->
                                    <div class="department_select_container" @mouseleave="departmentEnter(false,$event)" @mouseenter="departmentEnter(true,$event)">
                                        <input style="border:none" v-if="buslines.length === 0" disabled type="text" class="input-text" value="无可选择线路">
                                        <input style="border:none" v-else autocomplete="off" @click="departmentClick($event)" v-model="selectLineName" type="text" class="input-text" placeholder="请选择线路" name="department">
                                        ...
                                        <div class="departmentListBox" style="display: none">
                                            <div v-bind:title="item" v-for="item in buslines | filterBy selectLineName in 'shuttle_name'" @click="setBusLineRange(item)" class="departmentListItem">{{item.shuttle_name}}</div>
                                        </div>
                                    </div>
                                <!--</div>-->
                            <!--</div>-->
                        ...
                        <!--<select id="selectLine" class="select bgc-f4" name="demo1" v-model="selectBusLine" @change="setBusLineRange($event)">-->
                            <!--<option value="{{bus.id}}" :selected="bus.id == selectBusLine" v-for="bus in buslines">{{bus.shuttle_name}}</option>-->
                            <!--&lt;!&ndash;<option value="{{bus.id}}" v-for="bus in buslines">{{bus.shuttle_name}}:&nbsp;&nbsp;{{bus.stations_json[0].station_name}}&#45;&#45;&#45;&#45;{{bus.stations_json[bus.stations_json.length-1].station_name}}</option>&ndash;&gt;-->
                        <!--</select>-->
                    </span>
                </div>
                <div id="map"></div>
                <div class="location_search_box">
                    <input v-on:click="stopPro($event)" v-on:keydown="keydownToSearch($event)" v-on:focus="focusToSearch($event)" v-model="searchInfo" type="text" placeholder="请输入地名检索" id="search_input" size="20" class="search_input f-l">
                    <div v-on:click="searchLocation" class="f-r searchBtn"><i class="Hui-iconfont">&#xe665;</i></div>
                </div>
                <div class="result_box">
                    <div class="result_item clearfix" v-for="(index, item) in searchListArr" v-on:click="select($event, item)">
            ...
            <div class="win_point_info">
                ...
                <div class="win_point_info_station">
                    ...
                    <div class="available_station_list">
                            <div v-for="(index, item) in noCoordinateStationArr" class="available_station_item" value="{{item.id + ',' + item.station_name + ',' + item.tempId}}" title="{{item.station_name}}">{{item.station_name}}</div>
...
<script type="text/ecmascript-6">
    ...
    import * as drag from '../assets/js/jquery-drag.js';
...
//                            console.log("map loaded");
                            ...
                            that.stationIcon = new BMap.Icon("/static/images/icon-station.png", new BMap.Size(35,35),{imageSize: new BMap.Size(35,35), anchor: new BMap.Size(17, 35)});
                            // 显示已经标记的站点
...
//                console.log($event);
            ...
            getBuslines: function(callback=function(){}){
                let that = this;
                let ajaxData = {
                    page:1,
                    page_size:999999,
            ...
            getShuttlebusesStations: function(callback=function(){}){
                let that = this;
                $.ajax({
                    url: `${base.gAjaxUrl.shuttlebuses3}${that.selectBusLine}/stations/`,
                    type: 'GET',
                    data: {
...
//               var  $(e.target.parentNode).attr("index")
            ...
            add:function () {
                var newStation;
                    ...
                    newStation = $.trim($(".newStation").val());
                    $(".newStation").val('');
