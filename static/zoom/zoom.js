...
(function( $ ){
  ...
  $.fn.smartZoom = function(method) {
    ...
    var publicMethods = {
      ...
      init : function(options) {
        ...
        var settings = $.extend( {
          ...
          'initCallback' : null,
          'maxScale' : 3,
          'minScale' : 0.3, // 自己添加的缩小属性, 会有bug，暂时不用了
      ...
      zoom : function(scaleToAdd, globalRequestedPosition, duration){
        ...
        // newScale = Math.max(smartData.settings.minScale, newScale); // 增加一个最小的缩放量，默认值0.3
        ...
        dispatchSmartZoomEvent(ESmartZoomEvent.ZOOM, ESmartZoomEvent.START, false);
          ...
          // console.log(validPosition.x, validPosition.y)
    ...
    function adjustToContainer(){
      ...
      //   console.log(smartData.adjustedPosInfos.left , smartData.adjustedPosInfos.top, newWidth, newHeight)
      animate(targetElement,  smartData.adjustedPosInfos.left , smartData.adjustedPosInfos.top, newWidth, newHeight, 300, function() {
    ...
    function getBrowserTransitionObject(){
      ...
      for(var i=0; i<length; i++){ // for all kind of css transition we make a test
        if(bodyStyle[transitionTestArr[i]] != null){
          var transformStr = transformArr[i];
          ...
          var css3dSupported = ((div.offset().left - $('body').offset().left) == 20); // if translate3d(20px,0,0) via transformArr[i] == 20px the transformation is valid for this browser
          div.empty().remove();
